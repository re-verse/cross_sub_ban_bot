...
# try base64 decode, then JSON, else raw JSON
try:
...
def is_forgiven(user):
    for r in sheet.get_all_records():
        if (r.get('Username','').lower() == user.lower() 
         and str(r.get('ManualOverride','')).lower() in ('yes','true')):
            return True
    return False
...
def apply_override(username, moderator, modsub):
    ...
    for i, r in enumerate(records, start=2):
        if r.get('Username','').lower() == username.lower():
            ...
            sheet.update_cell(i, 7, moderator)   # OverriddenBy
            sheet.update_cell(i, 8, modsub)      # ModSub
            sheet.update_cell(i, 9, datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S'))  # ForgiveTimestamp
            return True
...
def check_modmail_for_overrides():
    ...
    for sub in TRUSTED_SUBS:
        ...
        try:
            ...
            print(f"[WARN] Couldn’t read modmail in r/{sub}, skipping.")
    ...
    try:
        ...
        for log in sr.mod.log(action='banuser', limit=50):
            ...
            if already_logged_action(lid):
                continue
            if get_recent_sheet_entries(src) >= DAILY_BAN_LIMIT:
                continue
            sheet.append_row([user, src, CROSS_SUB_BAN_REASON, ts.strftime('%Y-%m-%d %H:%M:%S'), '', lid, '', '', ''])
...
def enforce_bans_on_sub(sub):
    ...
    for r in records:
        ...
        ul = user.lower()
        # skip marked-deleted users
        if r.get('ForgiveTimestamp','').endswith('deleted'):
        ...
        # SKIP already banned/exempt/mod
